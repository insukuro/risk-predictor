...
export const DynamicForm: React.FC<DynamicFormProps> = ({
  ...
  // Find pump field for highlighting
  ...
  // Render pump field prominently at the top
  const renderPumpHighlight = () => {
    if (!highlightPump || !pumpField) return null;

    const pumpValue = values[pumpField.id];
    const isOn = pumpValue === 1 || pumpValue === true;

    return (
      <Card 
        variant={isOn ? 'danger' : 'default'}
        className="mb-6"
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <AlertCircle className={cn(
                'h-6 w-6',
                isOn ? 'text-red-600' : 'text-slate-400'
              )} />
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {isOn 
                    ? 'Операция с ИК — повышенный риск осложнений'
                    : 'Операция без ИК (off-pump)'
                  }
                </p>
              </div>
            </div>
    ...
    <div className="space-y-6">
      ...
      {formBlocks.map((block) => {
        const visibleBlockFields = block.fields
          .filter(f => {
            // Skip pump field in blocks if highlighted separately
            if (highlightPump && f.id.toLowerCase().includes('pump')) {
              return false;
            }
            if (visibleFields !== null) {
              return visibleFields.includes(f.id);
            }
            return true;
          })
